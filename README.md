...
> Apple and MacBook are trademarks of Apple Inc. Product facts come from official Apple UK
...
Optional or inapplicable components are omitted and the score is normalized over the applied weight. 
Ties resolve deterministically through ranking score, total fit, component order, number of
...
Prices are snapshots verified on 31 July 2026 and are subject to change. Product facts are Apple-sourced;
...
The production application has no dependencies. Playwright is an exact-version development-only dependency.
...
Northstar v1.1.0 is the current stable release and is deployed to production through GitHub Pages from `main`.

Version 1.1 introduces a streamlined adaptive questionnaire, richer multi-select workload inputs, simpler user-facing wording, explicit must-have requirements, improved Exact / Closest / Stretch / No Match handling, compact answer review, individual answer editing, accessible recommendation comparison and expanded automated browser testing.

Northstar v1.0.0 remains available as the previous tagged release.

Core v1.1 development is complete. Optional shareable-result functionality remains deferred to a future release. 
